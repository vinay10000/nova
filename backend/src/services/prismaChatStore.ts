...
import { ConversationNotFoundError, type ChatStore } from './chatService.js';
...
export function prismaChatStore(db: PrismaClient): ChatStore {
  return {
    ...
    async titleConversation(conversationId, userId, title) {
      // updateMany with userId scope = isolation even on this fire-and-forget path.
      await db.conversation.updateMany({ where: { id: conversationId, userId }, data: { title } });
    },
  };
}
